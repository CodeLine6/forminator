...
import { ElementsType, FormElementInstance, FormElementsType } from '@/components/FormElements';
...
async function SubmissionsTable({id} : {id: number}) {
  ...
  formElements.forEach((element: FormElementInstance) => {
    switch(element.type) {
        case "TextField":
          case "NumberField":
          case "TextAreaField":
        ...
        <TableBody>
          {rows.map((row) => (
            <TableRow key={row.id}>
              ...
              <TableCell className='text-right text-muted-foreground'>{formatDistance(row.submittedAt,new Date(), {
                addSuffix: true
              })}</TableCell>
            </TableRow>
          ))}
